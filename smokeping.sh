...
#定义文件路径
smokeping_ver="/usr/local/smokeping/onekeymanage/ver"
smokeping_key="/usr/local/smokeping/onekeymanage/key"
smokeping_name="/usr/local/smokeping/onekeymanage/name"
smokeping_host="/usr/local/smokeping/onekeymanage/host"
...
Configure_SomkePing(){
	...
	wget -O /usr/local/smokeping/lib/Smokeping/Graphs.pm https://raw.githubusercontent.com/ArronYin/smokeping-onekey/master/Graphs.pm
...
Change_Access(){
	...
	chown -R nginx:nginx /usr/local/smokeping/etc/smokeping_secrets.dist
}

#设置Slaves密钥
Slaves_Set_Secret(){
	rm -rf /usr/local/smokeping/etc/smokeping_secrets.dist
	echo -e "${slaves_secret}" > /usr/local/smokeping/etc/smokeping_secrets.dist
}
...
Slaves_Run_SmokePing(){
	cd /usr/local/smokeping/bin
	./smokeping --master-url=http://$server_name/smokeping.fcgi --cache-dir=/usr/local/smokeping/htdocs/cache --shared-secret=/usr/local/smokeping/etc/smokeping_secrets.dist --slave-name=$slaves_name --logfile=/usr/local/smokeping/slave.log
}
